 ...
 *
 * When the ladder cannot restore health - or when the action Doctor believes is
 ...
 * BUILT. There is no credential-purge code path anywhere in Doctor. On a suspected
 ...
 * what Doctor wanted to do without it ever touching `~/.deeplake/credentials.json`.
 *
...
/**
 * The action Doctor recommends a human (or 064g) take, INCLUDING the deferred one
...
export interface EscalationRecord {
	...
	/** The action Doctor recommends next (may be the deferred credential purge). */
	readonly recommendedAction: RecommendedAction;
	/**
	 * When `recommendedAction` is a DEFERRED action Doctor did not perform, a
...
/**
 * The deferred actions Doctor is allowed to RECOMMEND but never PERFORM. Centralized
 ...
 * states the action Doctor declined to take. Pure: no I/O, no throw.
 */
