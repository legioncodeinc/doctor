# PRD-002a: The hivedoctor to the-hive SSE producer
...
Implement the single Server-Sent-Events producer that streams hivedoctor's in-memory source-of-truth model to the-hive in near real time. This realizes [`ADR-0001`](../../../knowledge/private/architecture/ADR-0001-hive-telemetry-transport-and-single-source-of-truth.md) decision 3: exactly one SSE stream, hivedoctor to the-hive, feeding the health rail, the `/buzzing` readiness screen, and the health page. There is no service-to-hivedoctor SSE and no other streaming surface.

The producer reads the merged model that [PRD-001c](../prd-001-service-registration-and-telemetry-ingestion/prd-001c-service-registration-and-telemetry-ingestion-poll-merge-loop.md) maintains and emits well-defined events for fleet health and the enumerated metrics. It is served with `node:http` only, keeping hivedoctor dependency-light and crash-proof, and it is fail-soft: a disconnecting portal, a slow consumer, or an unavailable service database must never wedge or crash hivedoctor.

---

## Goals

- Serve exactly one SSE stream from hivedoctor to the-hive.
...
- Deliver near real time: portal state trails hivedoctor's model by about one poll interval.
...
- The portal-side rendering (the-hive forthcoming PRD-004 / PRD-005).
- Any inbound stream from services to hivedoctor.
...
- As the-hive portal, I open one SSE connection to hivedoctor and receive near-real-time fleet health and metrics without polling each daemon.
- As an operator watching `/buzzing`, I see readiness update within about a second of a service coming up or going down.
- As hivedoctor, when the portal drops its connection, I clean up and keep running; I never crash because a consumer left.
...
- **Fleet health:** per-service health and the coarse status hivedoctor already computes (`ok | degraded | unreachable | unknown`), plus disconnect/last-seen state from PRD-001c.
...
| ID | Criterion |
|---|---|
| a-AC-1 | Given the portal connects, when hivedoctor serves telemetry, then there is exactly one SSE stream from hivedoctor to the-hive and no other streaming surface. |
...
| a-AC-5 | Given the portal disconnects or consumes slowly, when that happens, then hivedoctor cleans up the connection and keeps running (fail-soft). |
...
- Serve alongside the existing status page at `hivedoctor/src/status-page/server.ts` (`:3852`), reusing the loopback server rather than adding a new listener where practical.
- Read straight from the PRD-001c in-memory model; do not re-open SQLite in the producer.
- Emit bounded log slices (per 002c) rather than whole histories to keep both hivedoctor and the portal memory bounded.
...
- the-hive [`ADR-0004-portal-landing-gate-and-path-based-routing`](../../../../../the-hive/library/knowledge/private/architecture/ADR-0004-portal-landing-gate-and-path-based-routing.md) - the portal routes that consume this stream.
