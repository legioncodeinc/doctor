...
Keep memory and disk bounded so logs never balloon under sustained logging. This realizes [`ADR-0001`](../../../knowledge/private/architecture/ADR-0001-hive-telemetry-transport-and-single-source-of-truth.md)'s bounded-memory decision driver from both ends: doctor reads windows (recent rows and aggregates) rather than loading whole logs, and services cap and rotate their telemetry databases so the underlying files stay small.

ADR-0001 states memory stays bounded because doctor queries windows and the portal pages request bounded slices, and it lists "doctor must manage many SQLite readers and be disciplined about windowed queries" as a negative consequence to manage. This sub-PRD makes that discipline explicit and adds the retention and rotation policy that keeps the write side bounded too, so a chatty service cannot grow an unbounded log file.

---

## Goals

- Windowed reads: doctor reads only recent rows and aggregates, never whole log histories, so its memory stays bounded regardless of how much a service has logged.
...
- The policy holds under sustained high-volume logging without unbounded growth in doctor memory or on disk.
...
- As doctor, no matter how much a service logs, my memory footprint for reading its telemetry stays roughly constant because I only ever query a bounded window.
...
1. **Windowed reads (reader side).** doctor's queries always bound their result set (recent rows within a time or count window, or aggregates). No query loads a whole log table.
2. **Bounded slices (stream side).** Log payloads over SSE are chunked into bounded slices so neither doctor nor the portal holds an unbounded history.
...
4. **Sustained-load safety.** Under continuous logging, doctor memory and the on-disk database both stay within their bounds.
...
| ID | Criterion |
|---|---|
| c-AC-1 | Given a service with a large log history, when doctor reads it, then the query returns only a bounded window (recent rows or aggregates) and doctor memory does not grow with total log volume. |
...
| c-AC-4 | Given sustained high-volume logging over time, when the system runs, then doctor memory and the on-disk telemetry database both stay bounded. |
...
- The reader-side window (doctor) and the writer-side cap (services) are independent defenses; both are required. Windowed reads protect doctor even if a writer's cap misbehaves, and the cap protects the disk even if a reader over-fetches.
...
- Rotation should be crash-safe and WAL-friendly so a rotate that races with doctor's read never corrupts the database or wedges the reader.
...
- [ ] Whether rotation is delete-in-place (prune rows) or file rotation, given doctor holds read-only handles.
