...
- As doctor's poll loop, I read each entry's database path(s) so I know which SQLite files to open read-only.
...
- As the installer, I append one optional field to an entry without breaking doctor's parser.
...
| ID | Criterion |
|---|---|
| a-AC-1 | Given an entry with a SQLite database path field, when the registry is parsed, then doctor records that path (or list of paths) on the in-memory entry. |
...
| a-AC-4 | Given a malformed registry file, when parsed, then doctor falls back and surfaces a needs-attention record and does not crash-loop. |
...
- Extend the registry loader/parser at `doctor/src/registry.ts` (the loader ADR-0002 cites). Keep the additive field optional in the type and default it to an empty list.
...
- nectar [`PRD-004`](../../../../../nectar/library/requirements/backlog/prd-004-doctor-registry-and-hive/prd-004-doctor-registry-and-hive-index.md) - PRD-004a's registry and parser this extends.
